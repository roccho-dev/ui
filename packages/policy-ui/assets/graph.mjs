...
// node_modules/.pnpm/@roccho+graph-editor@file+..+graph-editor/node_modules/@roccho/graph-editor/src/model.mjs
var invalid = (message) => {
...
var exactKeys = (value, keys, location2) => {
  if (!plainObject(value)) invalid(`${location2} must be an object`);
  const expected = new Set(keys);
  for (const key of keys) if (!Object.hasOwn(value, key)) invalid(`${location2}.${key} is required`);
  for (const key of Object.keys(value)) if (!expected.has(key)) invalid(`${location2}.${key} is not supported`);
};
var textId = (value, location2) => {
  if (typeof value !== "string" || !value || value !== value.trim()) invalid(`${location2} must be a non-empty trimmed string`);
  return value;
};
var title = (value, location2) => {
  if (typeof value !== "string" || !value.trim()) invalid(`${location2} must be non-empty text`);
  return value;
};
var label = (value, location2) => {
  if (typeof value !== "string") invalid(`${location2} must be a string`);
  return value;
};
var finite = (value, location2) => {
  if (typeof value !== "number" || !Number.isFinite(value)) invalid(`${location2} must be finite`);
  return value;
};
var compareText = (left, right) => left === right ? 0 : left < right ? -1 : 1;
var typeRank = /* @__PURE__ */ new Map([["rect", 0], ["edge.arrow", 1]]);
var cellOrder = (left, right) => typeRank.get(left.type) - typeRank.get(right.type) || compareText(left.id, right.id);
var canonicalCells = (cells) => [...cells].sort(cellOrder);
var normalizeCell = (value, location2, canvas) => {
  if (!plainObject(value)) invalid(`${location2} must be an object`);
  if (value.type === "rect") {
    exactKeys(value, ["id", "type", "label", "x", "y", "width", "height"], location2);
    const cell = {
      id: textId(value.id, `${location2}.id`),
      type: "rect",
      label: label(value.label, `${location2}.label`),
      x: finite(value.x, `${location2}.x`),
      y: finite(value.y, `${location2}.y`),
      width: finite(value.width, `${location2}.width`),
      height: finite(value.height, `${location2}.height`)
    };
    if (cell.x < 0 || cell.y < 0 || cell.width <= 0 || cell.height <= 0) invalid(`${location2} must have positive size and non-negative position`);
    if (cell.x + cell.width > canvas.width || cell.y + cell.height > canvas.height) invalid(`${location2} must fit canvas`);
    return cell;
  }
  if (value.type === "edge.arrow") {
    exactKeys(value, ["id", "type", "label", "source", "target"], location2);
    return {
      id: textId(value.id, `${location2}.id`),
      type: "edge.arrow",
      label: label(value.label, `${location2}.label`),
      source: textId(value.source, `${location2}.source`),
      target: textId(value.target, `${location2}.target`)
    };
  }
  invalid(`${location2}.type must be rect or edge.arrow`);
};
var normalizeCells = (values, location2, canvas) => {
  if (!Array.isArray(values)) invalid(`${location2} must be an array`);
  const cells = values.map((value, index) => normalizeCell(value, `${location2}[${index}]`, canvas));
  const frame = /* @__PURE__ */ new Map();
  for (const cell of cells) {
    if (frame.has(cell.id)) invalid(`${location2} contains duplicate ${cell.id}`);
    frame.set(cell.id, cell);
  }
  for (const cell of frame.values()) {
    if (cell.type !== "edge.arrow") continue;
    if (frame.get(cell.source)?.type !== "rect") invalid(`${location2}: ${cell.id}.source does not reference a rect`);
    if (frame.get(cell.target)?.type !== "rect") invalid(`${location2}: ${cell.id}.target does not reference a rect`);
  }
  return canonicalCells(frame.values());
};
var normalizeDocument = (value) => {
  ...
  const maps = value.maps.map((map, mapIndex) => {
    const location2 = `root.maps[${mapIndex}]`;
    exactKeys(map, ["id", "title", "timeline"], location2);
    const mapId = textId(map.id, `${location2}.id`);
    if (mapIds.has(mapId)) invalid(`${location2}.id is duplicated`);
    mapIds.add(mapId);
    if (!Array.isArray(map.timeline) || map.timeline.length < 1) invalid(`${location2}.timeline must contain at least one state`);
    const stateIds = /* @__PURE__ */ new Set();
    const timeline = map.timeline.map((state, timeIndex) => {
      const stateLocation = `${location2}.timeline[${timeIndex}]`;
    ...
    return { id: mapId, title: title(map.title, `${location2}.title`), timeline };
  });
  return { schema: 3, canvas, maps };
};
var stringifyDocument = (value) => `${JSON.stringify(normalizeDocument(value), null, 2)}
`;
var normalizeActive = (document2, active) => {
  ...
  if (active.mapIndex < 0 || active.mapIndex >= document2.maps.length) invalid(`map index ${active.mapIndex} is out of range`);
  if (active.timeIndex < 0 || active.timeIndex >= document2.maps[active.mapIndex].timeline.length) invalid(`time index ${active.timeIndex} is out of range`);
  return { mapIndex: active.mapIndex, timeIndex: active.timeIndex };
};
var createState = (value, active = { mapIndex: 0, timeIndex: 0 }) => {
  const document2 = normalizeDocument(value);
  return { document: document2, active: normalizeActive(document2, active) };
};
var activeCells = (state) => state.document.maps[state.active.mapIndex].timeline[state.active.timeIndex].cells;
var editorView = (state) => {
...
var activeList = (cells) => canonicalCells(cells).map((cell) => ({
  id: cell.id,
  type: cell.type,
  label: cell.label,
  description: cell.type === "rect" ? `${cell.label || "(empty)"} \u2014 rect at ${cell.x}, ${cell.y}; ${cell.width} \xD7 ${cell.height}` : `${cell.label || "(empty)"} \u2014 arrow ${cell.source} \u2192 ${cell.target}`
}));
var reduce = (state, event) => {
  ...
  if (event.type === "replace-active-cells") {
    ...
    const document2 = normalizeDocument({ ...state.document, maps });
    return { document: document2, active: state.active };
  }
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/image/ImageBox.js
var ImageBox = class {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/event/EventObject.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/event/EventSource.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/Constants.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/Client.js
...
Client.IS_SVG = isWindowObjectAvailable() && navigator.appName?.toUpperCase() !== "MICROSOFT INTERNET EXPLORER";
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/EventUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/logger.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/internal/clone-utils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/i18n/provider.js
var NoOpI18n = class {
  isEnabled() {
    return false;
  }
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/config.js
var GlobalConfig = {
  /**
...
var defaultStyleDefaultsConfig = { ...StyleDefaultsConfig };

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/internal/utils.js
var doEval = (expression) => {
  return eval(expression);
};
var isElement = (node) => node?.nodeType === NODE_TYPE.ELEMENT;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/domUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/event/InternalMouseEvent.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/event/InternalEvent.js
...
var InternalEvent = class _InternalEvent {
   ...
   */
  static addListener(element2, eventName, funct) {
    element2.addEventListener(eventName, funct, supportsPassive ? { passive: false } : false);
    if (!element2.mxListenerList) {
      element2.mxListenerList = [];
    }
    const entry = { name: eventName, f: funct };
    element2.mxListenerList.push(entry);
  }
  /**
   * Removes the specified listener from the given element.
   */
  static removeListener(element2, eventName, funct) {
    element2.removeEventListener(eventName, funct, false);
    if (element2.mxListenerList) {
      const listenerCount = element2.mxListenerList.length;
      for (let i = 0; i < listenerCount; i += 1) {
        const entry = element2.mxListenerList[i];
        if (entry.f === funct) {
          element2.mxListenerList.splice(i, 1);
          break;
        }
      }
    }
   ...
   */
  static removeAllListeners(element2) {
    const list = element2.mxListenerList;
    if (list) {
      while (list.length > 0) {
        const entry = list[0];
        _InternalEvent.removeListener(element2, entry.name, entry.f);
      }
    }
  }
  /**
   ...
   */
  static release(element2) {
    try {
      _InternalEvent.removeAllListeners(element2);
      const children = element2.childNodes;
      if (children !== void 0) {
   ...
   */
  static disableContextMenu(element2) {
    _InternalEvent.addListener(element2, "contextmenu", (evt) => {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/geometry/Point.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/geometry/Rectangle.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellPath.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/styleUtils.js
var getCurrentStyle = (element2) => {
  return !element2 || element2.toString() === "[object ShadowRoot]" ? null : window.getComputedStyle(element2, "");
};
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/StringUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/ObjectIdentity.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/cloneUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/Cell.js
var Cell = class {
  ...
  hasAttribute(name) {
    ...
    return isElement(userObject) && userObject.hasAttribute ? userObject.hasAttribute(name) : !isNullish(userObject.getAttribute?.(name));
  }
  /**
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/ChildChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/GeometryChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/RootChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/StyleChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/TerminalChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/ValueChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/index.js
var edge_exports = {};
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellState.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/mathUtils.js
var toRadians = (deg) => {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/SideToSide.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/TopToBottom.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/Elbow.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/config.js
...
var originalOrthogonalConnectorConfig = { ...OrthogonalConnectorConfig };
var allDirections = () => {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/EntityRelation.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/Loop.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/arrayUtils.js
var equalPoints = (a, b) => {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/geometry/Geometry.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/shared.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/Segment.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/Orthogonal.js
var dirVectors = [
  [-1, 0],
  [0, -1],
  [1, 0],
...
function getJettySize(state, isSource) {
  ...
  if (value === "auto") {
    ...
    if (type !== NONE) {
      const size = (isSource ? state.style.startSize : state.style.endSize) ?? StyleDefaultsConfig.markerSize;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/Manhattan.js
var ManhattanConnector = (state, source, target, points, result) => {
  ...
  class ObstacleMap {
    ...
    build(source2, target2) {
      const graph = source2?.view.graph || target2?.view.graph;
      if (!graph)
        return;
  ...
  function alignPointToCell(point, edgeState, cellState, isSourceCell) {
    ...
    if (y != void 0 && onlyHorizontalDirections) {
      ...
      point.y = cellBounds?.y != void 0 ? cellBounds?.y + cellHeight * y : point.y - cellHeight / 2 + cellHeight * y;
    }
    ...
    if (x != void 0 && onlyVerticalDirections) {
      ...
      point.x = cellBounds?.x != void 0 ? cellBounds?.x + cellWidth * x : point.x - cellWidth / 2 + cellWidth * (x || 0);
    }
  }
  ...
  router(state, source, target, points, result, config2);
};

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/index.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/EllipsePerimeter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/HexagonPerimeter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/RectanglePerimeter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/RhombusPerimeter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/TrianglePerimeter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/marker/edge-markers.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/internal/BaseRegistry.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/edge/EdgeStyleRegistry.js
var EdgeStyleRegistryImpl = class extends BaseRegistry {
  ...
  add(name, edgeStyle2, metaData) {
    super.add(name, edgeStyle2);
    metaData?.handlerKind && this.handlerMapping.set(edgeStyle2, metaData.handlerKind);
    !isNullish(metaData?.isOrthogonal) && this.orthogonalStates.set(edgeStyle2, metaData.isOrthogonal);
    !isNullish(metaData?.allowIntermediateHandles) && this.intermediateHandlesStates.set(edgeStyle2, metaData.allowIntermediateHandles);
  }
  isOrthogonal(edgeStyle2) {
    return this.orthogonalStates.get(edgeStyle2) ?? false;
  }
  getHandlerKind(edgeStyle2) {
    return this.handlerMapping.get(edgeStyle2) ?? "default";
  }
  allowsIntermediateHandles(edgeStyle2) {
    return this.intermediateHandlesStates.get(edgeStyle2) ?? true;
  }
  clear() {
    super.clear();
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellHighlight.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellMarker.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/UrlConverter.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/canvas/AbstractCanvas2D.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/xmlUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/canvas/SvgCanvas2D.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/Shape.js
var Shape = class {
  ...
  getArcSize(w, h) {
    ...
    const roundingFactor = (this.style?.arcSize ?? StyleDefaultsConfig.roundingFactor * 100) / 100;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/node/EllipseShape.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/node/RectangleShape.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/node/ImageShape.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/other/ConnectionConstraint.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/ConstraintHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/config.js
...
var defaultEdgeHandlerConfig = { ...EdgeHandlerConfig };
var HandleConfig = {
  /**
...
var defaultVertexHandlerConfig = { ...VertexHandlerConfig };

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/EdgeHandler.js
var EdgeHandler = class {
  ...
  isHandleVisible(index) {
    ...
    const edgeStyle2 = geo ? this.graph.view.getEdgeStyle(this.state, geo.points || void 0, source, target) : null;
    return EdgeStyleRegistry.allowsIntermediateHandles(edgeStyle2) || index === 0 || index === this.abspoints.length - 1;
  }
  /**
   * Creates the shape used to display the given bend.
   * Note that the index
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/VertexHandler.js
  ...
  start(x, y, index) {
    ...
    if (this.index <= InternalEvent_default.CUSTOM_HANDLE && this.isGhostPreview()) {
      ...
      if (!this.livePreviewActive || this.isLivePreviewBorder()) {
        ...
        if (!(Client_default.IS_SVG && (this.state.style.rotation ?? 0) != 0) && this.state.text != null && this.state.text.node.parentNode === this.graph.container) {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/internal/i18n-utils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/ElbowEdgeHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/handler/EdgeSegmentHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/util/cellArrayUtils.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/CellsMixin.js
var CellsMixin = {
  cellsResizable: true,
  cellsBendable: true,
  ...
  postProcessCellStyle(style) {
    ...
    let image = this.getPlugin("image-bundle")?.getImageFromBundles(key) ?? null;
    if (image) {
      style.image = image;
    } else {
      image = key;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/ConnectionsMixin.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/DragDropMixin.js
var DragDropMixin = {
  dropEnabled: false,
  splitEnabled: true,
  autoScroll: true,
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/EdgeMixin.js
var EdgeMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/EditingMixin.js
var EditingMixin = {
  cellsEditable: true,
  /*****************************************************************************
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/EventsMixin.js
var EventsMixin = {
  // TODO: Document me!
  lastTouchEvent: null,
  ...
  fireMouseEvent(evtName, me, sender) {
    ...
    if (!this.isEventIgnored(evtName, me, sender)) {
      ...
      const cellEditorHandler = this.getPlugin("CellEditorHandler");
      if (evtName === InternalEvent_default.MOUSE_DOWN && this.isEditing() && !cellEditorHandler?.isEventSource(me.getEvent())) {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/FoldingMixin.js
var FoldingMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/GroupingMixin.js
var GroupingMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/LabelMixin.js
var LabelMixin = {
  getLabel(cell) {
    let result = "";
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/OrderMixin.js
var OrderMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellOverlay.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/OverlaysMixin.js
var OverlaysMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/edge/PolylineShape.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/PageBreaksMixin.js
var PageBreaksMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/PanningMixin.js
var PanningMixin = {
  shiftPreview1: null,
  shiftPreview2: null,
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/PortsMixin.js
var PortsMixin = {
  portsEnabled: true,
  isPort(cell) {
    return false;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/SelectionMixin.js
var SelectionMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/SnapMixin.js
var SnapMixin = {
  snapTolerance: 0,
  getSnapTolerance() {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/SwimlaneMixin.js
var SwimlaneMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/TerminalMixin.js
var TerminalMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/ValidationMixin.js
var ValidationMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/VertexMixin.js
var VertexMixin = {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/ZoomMixin.js
var ZoomMixin = {
  zoomFactor: 1.2,
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/mixin/_graph-mixins-apply.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/AbstractGraph.js
var AbstractGraph = class extends EventSource_default {
  ...
  createHandler(state) {
    ...
    if (state.cell.isEdge()) {
      ...
      const edgeStyle2 = this.getView().getEdgeStyle(state, geo ? geo.points || void 0 : void 0, source, target);
      result = this.createEdgeHandler(state, edgeStyle2);
    } else {
   ...
   */
  createEdgeHandler(state, edgeStyle2) {
    const handlerKind = EdgeStyleRegistry.getHandlerKind(edgeStyle2);
    switch (handlerKind) {
      case "elbow":
  ...
  isOrthogonal(edge) {
    ...
    const edgeStyle2 = this.view.getEdgeStyle(edge);
    return EdgeStyleRegistry.isOrthogonal(edgeStyle2);
  }
  /*****************************************************************************
   * Group: Graph appearance
   *****************************************************************************/
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/UndoableEdit.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/CollapseChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/VisibleChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/GraphDataModel.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/marker/EdgeMarkerRegistry.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/edge/ConnectorShape.js
var ConnectorShape = class extends PolylineShape_default {
  ...
  createMarker(c, pts, source) {
    ...
    if (type !== NONE && p0 !== null && pe !== null) {
      ...
      const unitX = dx / dist;
      const unitY = dy / dist;
      const size = (source ? this.style.startSize : this.style.endSize) ?? StyleDefaultsConfig.markerSize;
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/node/TextShape.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/ShapeRegistry.js
var ShapeRegistry = new BaseRegistry();

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/shape/stencil/StencilShapeRegistry.js
var StencilShapeRegistry = new BaseRegistry();

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/cell/CellRenderer.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/Stylesheet.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/SelectionChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/GraphSelectionModel.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/CurrentRootChange.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/style/perimeter/PerimeterRegistry.js
var PerimeterRegistry = new BaseRegistry();

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/GraphView.js
var GraphView = class extends EventSource_default {
  ...
  updatePoints(edge, points, source, target) {
    ...
    const edgeStyle2 = this.getEdgeStyle(edge, points, source, target);
    if (edgeStyle2 && source) {
      ...
      edgeStyle2(edge, src, trg, points, pts);
      if (src && srcBounds) {
  ...
  getEdgeStyle(edge, points = [], source = null, target = null) {
    let edgeStyle2 = this.isLoopStyleEnabled(edge, points, source, target) ? edge.style.loopStyle ?? this.graph.defaultLoopStyle : !(edge.style.noEdgeStyle ?? false) ? edge.style.edgeStyle : null;
    if (typeof edgeStyle2 === "string") {
      let tmp = EdgeStyleRegistry.get(edgeStyle2);
      if (!tmp && this.isAllowEval()) {
        tmp = doEval(edgeStyle2);
      }
      edgeStyle2 = tmp;
    }
    if (typeof edgeStyle2 === "function") {
      return edgeStyle2;
    }
    return null;
  }
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/BaseGraph.js
var BaseGraph = class extends AbstractGraph {
  initializeCollaborators(options) {
    this.cellRenderer = options?.cellRenderer ?? new CellRenderer_default();
    this.model = options?.model ?? new GraphDataModel_default();
    this.setSelectionModel(options?.selectionModel?.(this) ?? new GraphSelectionModel_default(this));
    this.setStylesheet(options?.stylesheet ?? new Stylesheet());
    this.view = options?.view?.(this) ?? new GraphView_default(this);
  }
};

// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/plugin/CellEditorHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/plugin/SelectionCellsHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/plugin/ConnectionHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/other/Guide.js
var Guide = class {
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/plugin/SelectionHandler.js
...
// node_modules/.pnpm/@maxgraph+core@0.24.0/node_modules/@maxgraph/core/lib/esm/view/undoable-change/UndoManager.js
...
// node_modules/.pnpm/@roccho+graph-editor@file+..+graph-editor/node_modules/@roccho/graph-editor/src/editor.mjs
var vertexStyle = {
  perimeter: "rectanglePerimeter",
  rounded: true,
  arcSize: 12,
...
var createEditor = ({ container, onChange, onSelectionChange }) => {
  PerimeterRegistry.add("rectanglePerimeter", perimeter_exports.RectanglePerimeter);
  EdgeMarkerRegistry.add("classic", edge_markers_exports.createArrow(2));
  const graph = new BaseGraph({
    container,
    plugins: [CellEditorHandler_default, ConnectionHandler_default, SelectionCellsHandler_default, SelectionHandler_default]
  });
  ...
  let hydrating = false;
  let metaByCell = /* @__PURE__ */ new WeakMap();
  let cellById = /* @__PURE__ */ new Map();
  const undoManager = new UndoManager_default(100);
  const registerCell = (runtime, logicalId, type) => {
    metaByCell.set(runtime, { id: logicalId, type });
    cellById.set(logicalId, runtime);
    return runtime;
  };
  const connection = graph.getPlugin(ConnectionHandler_default.pluginId);
  if (!connection) throw new Error("ConnectionHandler unavailable");
  connection.factoryMethod = () => {
    const runtime = new Cell("", new Geometry_default(), edgeStyle);
  ...
  const configure = (nextCanvas) => {
    ...
    graph.maximumGraphBounds = new Rectangle_default(0, 0, canvas.width, canvas.height);
  };
  const replaceCells = (cells) => {
    ...
    try {
      ...
      metaByCell = /* @__PURE__ */ new WeakMap();
      ...
      graph.batchUpdate(() => {
        ...
        for (const cell of cells.filter((value) => value.type === "edge.arrow")) {
          registerCell(graph.insertEdge({
            parent,
            value: cell.label,
            source: cellById.get(cell.source),
            target: cellById.get(cell.target),
            style: edgeStyle
          }), cell.id, cell.type);
        }
      });
  ...
  const insertRectangle = () => {
    const width = 160;
    const height = 56;
    if (canvas.width < width || canvas.height < height) throw new Error("Canvas is too small for a 160 \xD7 56 rectangle");
  ...
  graph.getDataModel().addListener(InternalEvent_default.UNDO, undoListener);
  graph.getSelectionModel().addListener(InternalEvent_default.CHANGE, selectionListener);
  undoManager.addListener(InternalEvent_default.UNDO, replayListener);
  undoManager.addListener(InternalEvent_default.REDO, replayListener);
  return {
    configure,
    replaceCells,
    readCells,
...
// node_modules/.pnpm/@roccho+graph-editor@file+..+graph-editor/node_modules/@roccho/graph-editor/src/component.mjs
var defaults = {
...
var requireCallback = (value, name) => {
  if (typeof value !== "function") throw new TypeError(`${name} must be a function`);
  return value;
};
var mountGraphEditor = (root, options = {}) => {
  ...
  const refresh = () => {
    ...
    frameOutput.value = `${view.mapIndex + 1} / ${view.mapCount} \xB7 ${view.mapTitle} \u2014 ${view.timeIndex + 1} / ${view.timeCount} \xB7 ${view.timeTitle}`;
    renderActiveList();
  };
  const synchronize = () => {
    editor.stopEditing();
  ...
  const commands = [
    ...
    {
      keys: [
        { key: "ArrowLeft", shift: "optional", dx: -1, dy: 0 },
        { key: "ArrowRight", shift: "optional", dx: 1, dy: 0 },
        { key: "ArrowUp", shift: "optional", dx: 0, dy: -1 },
        { key: "ArrowDown", shift: "optional", dx: 0, dy: 1 }
      ],
    ...
    { keys: [{ key: "s", command: true }], help: "save", enabled: () => Boolean(state && etag), action: save2 },
...
var save = async ({ document: value, etag }) => {
