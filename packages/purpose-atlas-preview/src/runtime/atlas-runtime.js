...
import {buildSnapshot, EVENTS, labelFromId} from '../domain/atlas-engine.js';
...
export class AtlasRuntime {
  constructor({onSurface, onError} = {}) {
    ...
    this.step = boundedStep(params.has('t') ? params.get('t') : 7);
    this.playing = false;
    this.viewMode = ['purpose', 'responsibility', 'focus'].includes(params.get('mode')) ? params.get('mode') : 'responsibility';
  ...
  async init() {
    try {
      ...
      this.publish('UIをキャッシュ描画で起動しました');
      return this.surface;
    } catch (error) {
      this.onError?.(error);
      throw error;
  ...
  syncFromDataModel() {
    ...
    this.viewMode = ['purpose', 'responsibility', 'focus'].includes(ui.viewMode) ? ui.viewMode : this.viewMode;
  ...
  dataModel() {
    ...
    return {
      meta: {
        ...
        subtitle: 'Purpose × Responsibility / latest source UI',
        protocol: 'A2UI v0.9',
  ...
  handleAction(action) {
    ...
    switch (action.name) {
      ...
      case 'atlas.modeChanged':
        this.viewMode = ['purpose', 'responsibility', 'focus'].includes(action.context?.mode) ? action.context.mode : this.viewMode;
