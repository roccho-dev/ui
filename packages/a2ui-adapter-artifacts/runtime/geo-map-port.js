export class GeoMapPort {
  ...
  update({properties = [], selection = null, poiKeys = [], onSelect = null} = {}) {
    ...
    for (const property of properties) {
      ...
      const marker = this.L.marker([lat, lng], {
        title: String(property.name || property.id),
        zIndexOffset: property.id === selectedId ? 900 : 0,
  ...
  renderNearby(property, keys) {
    ...
    for (const key of keys) {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) continue;
      this.L.marker([lat, lng], {title: String(poi.name || poi.label || key), zIndexOffset: 600}).addTo(this.layers.poi);
