...
function buildPropertyMap() {
  ...
  const sourceChecks = {
    surfaceHasGeoMap: surfaceText.includes('"port":"geoMap"'),
    surfaceHasPropertySelect: surfaceText.includes('"action":"property.select"'),
    registryHasGeoMap: registryText.includes('"geoMap"') && registryText.includes('GeoMapPort'),
    registryHasAtlasStage: registryText.includes('"atlasStage"'),
  ...
  const proof = {
    status: 'property-map-geo-artifact-pass',
    adapter,
    sourceChecks,
    files: {surface: 'dist/a2ui/property-map.surface.v0.9.jsonl', data: 'dist/a2ui/property-map.data.clear.v0.9.jsonl', registry: 'dist/registry/shared-component-registry.v1.json', runtime: 'runtime/geo-map-port.js', preview: 'preview/index.html'},
    boundaries: {sduiOwnsLayout: true, sharedRegistryOwnsContract: true, leafletIsolatedInsideGeoMapPort: true, adapterBuildDoesNotRenderMap: true},
  };
...
function makeInputShape(req, broken) { const shape = Object.fromEntries((req.inputContract?.requiredTopLevel || []).map((key) => [key, []])); if (broken === 'missing-input-shape') delete shape[req.inputContract.requiredTopLevel[0]]; return shape; }
function makeReceipt(adapter, projectionDigest, broken) { const receipt = {status: adapter === 'purpose' ? 'reduced' : 'closed', projectionDigest, closedGaps: adapter === 'purpose' ? ['gap-real-fixture-artifact', 'gap-purpose-proof-replayable'] : ['gap-live-adapter-artifact'], residuals: adapter === 'purpose' ? [{id: 'runtime-state-loop-not-in-scope', owner: 'consumer-runtime', next: 'external runtime closes live state loop'}] : [], residualHandling: {returnPath: 'ADRS/raw JSONL next projection', owner: 'ADRS', nextProjection: 'purpose closure projection'}}; if (broken === 'missing-residual-handling') delete receipt.residualHandling; return receipt; }
function validateProof(adapter, req, proof, inputShape) { const missing = (req.inputContract?.requiredTopLevel || []).filter((key) => !(key in inputShape)); if (!proof.source?.digest?.startsWith('sha256:')) throw new Error(adapter + ' proof missing source digest'); if (!proof.projection?.digest?.startsWith('sha256:')) throw new Error(adapter + ' proof missing projection digest'); if (missing.length) throw new Error(adapter + ' proof missing input shape: ' + missing.join(', ')); if (proof.workOrder.currentProjectionDigest !== proof.projection.digest) throw new Error(adapter + ' proof has stale work order projection digest'); if (proof.receipt.residuals.length && !proof.receipt.residualHandling) throw new Error(adapter + ' proof missing residual handling'); if (!proof.checkedOutputs.includes(`proof/${adapter}-adapter-proof-report.json`)) throw new Error(adapter + ' proof output list missing proof report'); }
