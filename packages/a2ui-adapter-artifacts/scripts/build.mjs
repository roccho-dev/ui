...
function brokenRoot(root, broken) {
  if (!['unexpected-action', 'unexpected-port'].includes(broken)) return root;
  return withExtra(root, broken === 'unexpected-action' ? {type: 'button', text: 'bad action', action: 'live.disallowedAction'} : {type: 'port', port: 'wrongStage'});
}
