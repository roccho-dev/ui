...
function applyDataRows(text) { const state = {}; for (const line of text.split(/\r?\n/).filter(Boolean)) { const msg = parseJson(line, 'property-map.data'); const update = msg.updateDataModel; if (!update) continue; setPath(state, update.path, update.value); } return state; }
