...
function bootRepoMapApp(root) {
  ...
  function update() { const z = zFromZoom(); draw(projectGraph(graph, { z, focusId: z >= 66 ? 'pkg:r04-p00' : 'repo:repo-04' }), instance.getZoom()); }
  function setZ(z) { instance.zoom(zoomFromZ(z)); update(); }
  ...
  root.querySelector('#fit').addEventListener('click', () => { instance.reset(); instance.fit(); instance.center(); update(); });
  function draw(view, zoom) {
    ...
    for (const node of view.nodes.sort((a,b) => a.depth - b.depth)) {
      ...
      nodeLayer.append(rect); const label = labelText(node); if (label) { const size = labelFontWorld(node, view.policy, view.camera.z, zoom); const text = svgEl('text', { class: 'label', 'data-role': node.role, 'data-for': node.id, x: r.x + 10, y: r.y + size * 1.2, 'font-size': size }, label); nodeLayer.append(text); }
    }
  ...
  const query = new URLSearchParams(location.search); const testCase = query.get('testCase'); if (testCase?.endsWith('z42')) setZ(42); else if (testCase?.endsWith('z80')) setZ(80); else { instance.fit(); instance.center(); update(); }
