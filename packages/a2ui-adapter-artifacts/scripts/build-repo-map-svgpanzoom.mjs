...
function bootRepoMapApp(root) {
  ...
  const instance = window.svgPanZoom(svg, { viewportSelector: '.svg-pan-zoom_viewport', panEnabled: true, zoomEnabled: true, mouseWheelZoomEnabled: true, dblClickZoomEnabled: true, controlIconsEnabled: false, fit: true, center: true, minZoom: 1, maxZoom: graph.policy.camera.svgPanZoomMaxZoom || 16, zoomScaleSensitivity: 0.25, onZoom: update, onPan: update });
  function zFromZoom() { const maxZoom = graph.policy.camera.svgPanZoomMaxZoom || 16; const maxZ = graph.policy.camera.maxZ || 100; return Math.round(Math.log(Math.max(1, instance.getZoom())) / Math.log(maxZoom) * maxZ); }
  function zoomFromZ(z) { const maxZoom = graph.policy.camera.svgPanZoomMaxZoom || 16; return Math.exp(Math.log(maxZoom) * Math.max(0, Math.min(100, z)) / 100); }
  function projectionForZ(z) { return projectGraph(graph, { z, focusId: z >= 66 ? 'pkg:r04-p00' : 'repo:repo-04' }); }
  function update() { const z = zFromZoom(); draw(projectionForZ(z), instance.getZoom()); }
  function setZ(z) { instance.zoom(zoomFromZ(z)); draw(projectionForZ(z), instance.getZoom()); }
  ...
  root.querySelector('#fit').addEventListener('click', () => { instance.reset(); instance.fit(); instance.center(); draw(projectGraph(graph, { z: 0 }), instance.getZoom()); });
  function draw(view, zoom) {
    ...
    for (const node of view.nodes.sort((a,b) => a.depth - b.depth)) {
      ...
      nodeLayer.append(rect); const label = labelText(node); if (label) { const size = labelFontWorld(node, view.policy, view.camera.z, Math.max(1, zoom)); const text = svgEl('text', { class: 'label', 'data-role': node.role, 'data-for': node.id, x: r.x + 10, y: r.y + size * 1.2, 'font-size': size }, label); nodeLayer.append(text); }
    }
    writeSmoke(root, svg, { z: view.camera.z, zoom, roles: countRoles(view.nodes), nodeCount: view.nodes.length, edgeCount: view.edges.length });
  }
  const query = new URLSearchParams(location.search); const testCase = query.get('testCase'); if (testCase?.endsWith('z42')) setZ(42); else if (testCase?.endsWith('z80')) setZ(80); else { instance.fit(); instance.center(); draw(projectGraph(graph, { z: 0 }), instance.getZoom()); }
