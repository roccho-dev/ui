...
const poiKeys = ['supermarket', 'mall', 'local_station', 'shinkansen_station', 'hospital', 'bus_stop', 'park', 'gymnasium', 'children_hall'];
const properties = createProperties(28, poiKeys);
...
const remountSnapshot = snapshot(port);

const checks = {
  mountCreatedMapOnce: initialMountCreates === 1,
  layerGroupsCreatedOncePerMount: initialLayerGroups === 3,
  hostTagged: host.getAttribute('data-geomap-port') === 'leaflet' && host.getAttribute('data-geomap-provider') === 'leaflet-osm',
  ...
  leafletOsmNoFallback: /^https:\/\/\{s\}\.tile\.openstreetmap\.org/.test(initialTileUrl) && !runtimeText.includes('about:blank') && !runtimeText.includes('dom-fallback') && !runtimeText.includes('createDomMapRuntime'),
...
function createProperties(count, keys) {
  return Array.from({length: count}, (_, index) => {
    const rank = index + 1;
    return {id: `p${rank}`, rank, name: `property ${rank}`, rent_yen: 40000 + rank * 1000, area_sqm: 40 + rank, geo: {lat: 36 + index * 0.01, lng: 139 + index * 0.01}, nearby: Object.fromEntries(keys.map((key, poiIndex) => [key, {label: key, lat: 36 + index * 0.01 + poiIndex * 0.001, lng: 139 + index * 0.01 + poiIndex * 0.001}]))};
  });
}
function createHost() {
  const attrs = new Map();
  return {children: [], setAttribute(name, value) { attrs.set(name, String(value)); }, getAttribute(name) { return attrs.get(name) ?? null; }, removeAttribute(name) { attrs.delete(name); }, appendChild(node) { this.children.push(node); node.host = this; return node; }, replaceChildren() { this.children.splice(0); }};
}
function createMockLeaflet() {
  const metrics = {mapCreates: 0, mapRemoves: 0, layerGroups: 0, tileUrls: []};
  return {
    metrics,
    map(host, options) { metrics.mapCreates += 1; return {host, options, fitBoundsCalls: 0, fitBounds() { this.fitBoundsCalls += 1; }, off() {}, remove() { metrics.mapRemoves += 1; host.children.splice(0); }}; },
    tileLayer(url, options) { metrics.tileUrls.push(url); return {url, options, addTo(map) { this.map = map; return this; }, on() { return this; }, remove() {}}; },
    latLngBounds() { return {pad() { return this; }}; },
    layerGroup(name) { metrics.layerGroups += 1; return {name, nodes: [], addTo(map) { this.map = map; return this; }, clearLayers() { for (const node of [...this.nodes]) node.remove(); this.nodes = []; }}; },
...
function layer(type, latlng, options) {
  return {type, latlng, options, events: {}, addTo(group) { const node = {type, latlng, options, events: this.events, remove() { const index = group.map.host.children.indexOf(node); if (index >= 0) group.map.host.children.splice(index, 1); }}; group.nodes.push(node); group.map.host.appendChild(node); return this; }, on(name, fn) { this.events[name] = fn; return this; }};
}
function snapshot(port) {
  return {properties: port.layers?.properties.nodes.length || 0, poi: port.layers?.poi.nodes.length || 0, radius: port.layers?.overlay.nodes.filter((node) => node.type === 'circle').length || 0, links: port.layers?.overlay.nodes.filter((node) => node.type === 'polyline').length || 0};
}
