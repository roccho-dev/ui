...
if (process.env.GEOMAP_ZIP_PARITY_INTERACTION === '1') await import('./render-geomap-interaction-parity.mjs');
console.log(JSON.stringify({status: 'zip-a2ui-parity-pass', activeProperties: checks.activeProperties, tabs: checks.tabs, components: oracle.components.length + '/' + registry.components.length, rendered: process.env.GITHUB_ACTIONS === 'true' || process.env.GEOMAP_ZIP_PARITY_RENDER === '1', interaction: process.env.GEOMAP_ZIP_PARITY_INTERACTION === '1'}, null, 2));
...
function label(key) { return {daily_use_rental_car: '格安レンタカー', supermarket: 'スーパー', mall: 'モール', local_station: '最寄駅', shinkansen_station: '新幹線', general_hospital: '病院', bus_stop: 'バス停', park: '公園', gymnasium: '体育館', children_hall: '児童館'}[key] || key; }
