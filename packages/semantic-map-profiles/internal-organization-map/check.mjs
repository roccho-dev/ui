...
const profile = JSON.parse((await fs.readFile(new URL('./profile.jsonl', import.meta.url), 'utf8')).trim());
...
const source = role => ({ role, kind: sourceKinds[role], sourceRef: `fixture://${role}@rev-1`, digest });
...
const entities = [
  {
    ...
    status: 'observed',
    sourceRef: repositories[0].sourceRef,
    authority: false,
  },
  {
    kind: 'organizationMap.entity.v1',
...
const bundle = {
  ...
  states: {
    contractGraph: {
      kind: 'contractGraph.current.v1', authority: false, repositories, entities,
      spines: [{
        kind: 'organizationMap.factorySpine.v1', id: 'spine:331:test',
        steps: [
          { from: 'decision:adrs:331', to: 'obligation:331:ops-staging', label: 'requires' },
          { from: 'obligation:331:ops-staging', to: 'package:roccho-dev/ops:artifact-assembly', label: 'feeds' },
        ],
      }],
    },
    decisionImpact: {
      kind: 'decisionImpact.current.v1', authority: false,
      decisions: [{
        kind: 'organizationMap.decision.v1', id: 'decision:adrs:331',
        title: 'Internal Organization Semantic Map', status: 'accepted',
        decisionRef: 'https://github.com/roccho-dev/adrs/issues/331',
        sourceRef: 'https://github.com/roccho-dev/adrs/pull/332', authority: false,
      }],
      obligations: [],
    },
    obligationState: {
      kind: 'obligationState.current.v1', authority: false,
      rows: [{
        kind: 'organizationMap.obligation.v1', id: 'obligation:331:ops-staging',
        decisionId: 'decision:adrs:331', ownerRepoId: 'roccho-dev/ops',
        title: 'Deploy staging map', status: 'pending',
        sourceRef: 'https://github.com/roccho-dev/adrs/issues/331', authority: false,
      }],
    },
    workCurrent: {
      kind: 'workLifecycle.current.v1', authority: false,
      rows: [{
        kind: 'organizationMap.work.v1', id: 'work:331:ops', repoId: 'roccho-dev/ops',
        status: 'pending', sourceRef: 'https://github.com/roccho-dev/ops/pull/360', authority: false,
      }],
    },
    responsibilityClosure: {
      kind: 'responsibilityClosure.current.v1', authority: false,
      rows: [{
        kind: 'organizationMap.responsibilityClosure.v1',
        id: 'closure:roccho-dev/ui:required-package', repoId: 'roccho-dev/ui',
        expectation: 'required-package', status: 'unknown', matchedPackageIds: [],
        inventoryState: 'unknown', authority: false,
      }],
    },
    evidenceState: { kind: 'evidenceState.current.v1', authority: false, rows: [] },
  },
};
...
assert.equal(projected.counts.repositories, 2);
...
const relations = new Map(projected.records.filter(row => row.type === 'relation').map(row => [row.id, row]));
for (const id of [
...
assert.ok([...regions.values()].some(row => row.label.includes('[UNKNOWN] package inventory')));
assert.ok(relations.has('spine:spine:331:test:0'));
assert.ok(relations.has('spine:spine:331:test:1'));
assert.ok([...relations.values()].every(row => regions.has(row.from) && regions.has(row.to)));
assert.ok(projected.records.filter(row => row.temporal).every(row => regions.has(row.temporal.actor)));
...
for (const mutate of [
  ...
  value => { delete value.states.evidenceState; value.inputs = value.inputs.filter(row => row.role !== 'evidenceState'); },
]) {
