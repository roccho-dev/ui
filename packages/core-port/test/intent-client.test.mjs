...
assert.doesNotMatch(source, /fetchImpl\s*\(/u, "semantic-intent adapter does not own raw browser fetch");
assert.match(source, /createJsonConnectability/u, "semantic-intent transport uses the single connectability output port");
...
assert.equal(serializeSemanticIntent(submission.intent), requestFixture);
...
const validFetch = async (url, options) => {
  ...
  return new Response(pendingFixture, {
    status: 200,
...
for (const [response, errorCode] of [
  [
    new Response("<p>invalid</p>", { status: 502, headers: { "content-type": "text/html" } }),
  ...
  [
    new Response(
      JSON.stringify({ ...JSON.parse(pendingFixture), intent_id: "intent-999" }),
