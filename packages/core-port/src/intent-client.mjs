...
function normalizeTargetRef(value) {
  ...
  return Object.freeze({
    kind: assertIdentifier(input.kind, "target_ref.kind"),
...
function normalizeIntent(value) {
  ...
  const normalized = {
    ...
    body: assertText(input.body, "intent.body"),
  };

  if (OWN.call(input, "topic_title")) {
    normalized.topic_title = assertText(input.topic_title, "intent.topic_title");
  }
...
export function createSemanticIntentSubmission(draft, { idFactory = defaultIdFactory } = {}) {
  ...
  const intent = {
    ...
    kind: SEMANTIC_INTENT_KIND,
    body: assertText(input.body, "draft.body"),
  };

  if (OWN.call(input, "topic_title")) {
    intent.topic_title = assertText(input.topic_title, "draft.topic_title");
  }
  if (OWN.call(input, "target_ref")) {
