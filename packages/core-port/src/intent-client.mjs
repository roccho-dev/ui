...
} from "../../connectability/src/index.mjs";
...
class SemanticIntentResponseError extends Error {
  constructor(code) {
    super(code);
    this.name = "SemanticIntentResponseError";
    this.code = code;
  }
}
...
export function serializeSemanticIntent(value) {
  const intent = normalizeIntent(value);
  const body = JSON.stringify(intent);
  if (utf8Length(body) > SEMANTIC_INTENT_LIMITS.requestBytes) {
    throw new TypeError(`intent exceeds ${SEMANTIC_INTENT_LIMITS.requestBytes} UTF-8 bytes`);
  }
  return body;
}
...
function normalizeSubmission(value) {
  ...
  const canonicalBody = serializeSemanticIntent(intent);
...
function responseGuard(response) {
  ...
  if (typeof contentType !== "string" || !/^application\/json(?:\s*;|$)/iu.test(contentType)) {
    throw new SemanticIntentResponseError("invalid_response_content_type");
  }
}

function connectionFor(fetchImpl) {
  return createJsonConnectability({
    endpoint: SEMANTIC_INTENT_ENDPOINT,
    fetch: fetchImpl,
    idOf: value => value.intent_id,
    maxBytes: SEMANTIC_INTENT_LIMITS.requestBytes,
    prepare: normalizeIntent,
    responseGuard,
    serialize: serializeSemanticIntent,
  });
}
...
export function createSemanticIntentSubmission(draft, { idFactory = defaultIdFactory } = {}) {
  ...
  return normalizeSubmission({ intent, requestBody: serializeSemanticIntent(intent) });
}
...
export async function submitSemanticIntent(submission, { fetchImpl = globalThis.fetch } = {}) {
  ...
  const connection = connectionFor(fetchImpl);
  const transportPrepared = await connection.prepare(prepared.intent);
  if (transportPrepared.bytes !== prepared.requestBody) {
    throw new TypeError("connectability changed semantic-intent request bytes");
  }

  let submitted;
  try {
    submitted = await connection.submit(transportPrepared);
  } catch (error) {
    if (error instanceof SemanticIntentResponseError) {
      return rejectedResult(prepared.intent.intent_id, error.code);
    }
    if (error instanceof ConnectabilityError) {
  ...
  let result;
  try {
    result = parseSemanticIntentResult(submitted.value, {
