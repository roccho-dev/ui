...
function edgeId(edge) { return edge.id || `${edge.source}__${edge.target}__${edge.kind || 'link'}`; }
...
function approvedSurface(surfaceText) {
  ...
  for (const token of ['"port":"atlasStage"', '"when":"selectedNode"', '"when":"notSelected"', 'G0閉包: 高価値法人構築と売却', 'nodeを選択すると詳細を表示', 'ADRS merge slide']) assert.ok(tree.includes(token), `approved UI token missing: ${token}`);
  for (const bad of ['"type":"segmented"', 'sdui-shell', 'Purpose Decision Atlas · SDUI']) assert.equal(tree.includes(bad), false, `approved UI must not include ${bad}`);
  for (const token of ['.app{', '.stage{', '.panel{', '.rail{', '.stage-port,.stage-port canvas{position:absolute;inset:0']) assert.ok(css.includes(token), `approved CSS token missing: ${token}`);
  return parsedRows;
}
...
for (const token of ['<purpose-atlas-app', 'purposeAtlasStandaloneFetch', 'A2uiSduiSurface']) assert.ok(html.includes(token), `uploaded A2UI-built HTML missing ${token}`);
for (const bad of ['sdui-shell', 'Purpose Decision Atlas · SDUI', 'source-ui.css']) assert.equal(html.includes(bad), false, `uploaded A2UI-built HTML must not include ${bad}`);
...
console.log(JSON.stringify({
  ...
  final: {nodes: final.nodes.length, edges: final.edges.length, activeGaps: final.activeGaps, archivedGaps: final.archivedGaps, kindCounts: final.kindCounts},
}, null, 2));
