...
assert.match(preview, /property-map-final-ui-overrides/);
...
assert.equal(report.checks.collapsedSheetVisible, true);
assert.equal(report.checks.openSheetVisible, true);
assert.equal(report.checks.detailSheetVisible, true);
assert.equal(report.checks.sheetAboveLeaflet, true);
assert.equal(report.checks.screenshotsDifferent, true);
...
assert.ok(report.checks.featuredMapLabelsVisible >= 4);
