...
assert.deepEqual(requirementNames, ['live']);
...
assert.deepEqual(indexedArtifacts, expectedArtifacts, 'generated artifact index must match active source requirements');
assert.deepEqual(generatedDirs, expectedArtifacts, 'generated artifact directories must match active source requirements');
assert.equal(indexedArtifacts.includes('purpose-adapter-artifact'), false);
assert.equal(fs.existsSync(path.join(tmp, 'purpose-adapter-artifact')), false);
