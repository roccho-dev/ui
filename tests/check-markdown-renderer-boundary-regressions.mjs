...
function assertForbiddenAuthorityField({ name, model, expectedPath }) {
  ...
  assert.equal(result.ok, false, `${name}: boundary fixture must block`);
...
const safeBaseModel = {
  ...
  blocks: [{ kind: "paragraph", text: "Renderer boundary fixture." }],
};
...
const forbiddenSourcePatterns = [
  ...
  ["artifact upload lifecycle", /uploadArtifact|actions\/upload-artifact|artifactLifecycleOwner/],
];
