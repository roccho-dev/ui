...
assert.match(html, /property-marker/);
assert.match(html, /property-core/);
assert.match(html, /map-label/);
assert.match(html, /dom-poi/);
assert.match(html, /dom-radius/);
assert.match(html, /dom-link/);
assert.match(html, /selected/);
assert.match(html, /position:absolute[^}]*display:flex[^}]*align-items:center[^}]*gap:8px/);
...
assert.ok(report.checks.propertyMarkersInitial >= 4);
assert.ok(report.checks.mapLabelsInitial >= 4);
assert.ok(report.checks.domPoiAfterClick >= 9);
assert.ok(report.checks.domRadiusAfterClick >= 1);
assert.ok(report.checks.domLinkAfterClick >= 9);
assert.equal(report.checks.absolutePropertyMarkerOverlay, true);
assert.equal(report.checks.textOnlyMarkerDetection, false);
assert.deepEqual(report.checks.requiredSelectors, ['.property-marker', '.property-core', '.map-label', '.selected', '.dom-poi', '.dom-radius', '.dom-link']);
...
console.log(JSON.stringify({status: 'geomap-canonical-ui-artifact-check-pass', propertyMarkers: report.checks.propertyMarkersInitial, mapLabels: report.checks.mapLabelsInitial, domPoi: report.checks.domPoiAfterClick, domRadius: report.checks.domRadiusAfterClick, domLink: report.checks.domLinkAfterClick}, null, 2));
