...
const contractPhrases = ["ADRS projected input", "adrs projected input", "core+port as lib", "a2ui as build", "jsonl as attached data", "stateless", "non-authoritative", "ui.git is not a state store"];
...
function assertContainsPhrases(label, text, phrases) { for (const phrase of phrases) assert.ok(text.includes(phrase), `${label} must contain ${phrase}`); }
function assertNoForbiddenKeys(label, value, trail = []) {
  if (!value || typeof value !== "object") return;
  if (Array.isArray(value)) { value.forEach((item, index) => assertNoForbiddenKeys(label, item, [...trail, index])); return; }
...
const actionNames = Object.entries(components[0]).filter(([key, value]) => key.startsWith("on") && value?.event?.name).map(([, value]) => value.event.name).sort();
assert.deepEqual(actionNames, ["atlas.clearSelection", "atlas.fit", "atlas.holdDecision", "atlas.modeChanged", "atlas.next", "atlas.previous", "atlas.recordMismatch", "atlas.requestOwner", "atlas.reset", "atlas.select", "atlas.stepChanged", "atlas.stepForward", "atlas.togglePlay", "atlas.zoomIn", "atlas.zoomOut"].sort());
...
for (const [file, expected] of Object.entries(sourceLock.sourceFilesSha256)) assert.equal(sha256File(path.join(sourceRoot, file)), expected, `source reference digest must match lock for ${file}`);
...
console.log(JSON.stringify({ status: "purpose-atlas-a2ui-check-pass", surface: "purpose-atlas", component: "AtlasSourceSurface", actions: actionNames.length }, null, 2));
