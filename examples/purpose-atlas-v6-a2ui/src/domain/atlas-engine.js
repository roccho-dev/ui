...
export function buildSnapshot(step, {zoom = 1} = {}) {
  ...
  for (const node of nodes) {
    // The latest source UI exposes responsibility-ring hit targets for every
    // visible node in responsibility mode. Serializing all branches keeps the
    // A2UI renderer semantically identical to that source implementation.
